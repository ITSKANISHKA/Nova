...
const placeOrder = asyncHandler(async (req, res) => {
  const { shippingAddress, paymentInfo, couponCode, discountAmount = 0 } = req.body;
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const validDiscount = Math.min(discountAmount, itemsTotal);
      const totalAmount = Math.max(0, itemsTotal + shippingFee - validDiscount);
